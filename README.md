...
Under REFramework `Script Generated UI`, Moveset Mod features a checkbox to enable a Moveset Research window. This window will appear during a match and allow you to trigger moves from the fighter's list, seek across the frames of their move, play in slow motion (or reverse), and seek frame by frame. 
...
- **edit_key / edit_obj** - takes a RE Managed object and a table of fields / properties vs values in which to change, and changes them
...
Search MovesetMod.lua for more information about these functions, as they are a part of the `data` class defined there. The only big thing we can't do here yet is add new TriggerGroups.
```
if not moves_by_id[905] then
    --Clone action ID #608 to a new action with ID #905:
    RYU_ACTION.NEW_5HP = data:clone_action(608, 905)
    
    --Clone HIT_DT_TBL ID #21 to a new HIT_DT_TBL with ID #1337 and add it to the new move RYU_ACTION.NEW_5HP
    local new_hit_dt_tbl = data:clone_dmg(21, 1337, RYU_ACTION.NEW_5HP)
    
    --Clone all triggers for ActionID #608 and add them to ActionID #905, then add to RYU_ACTION.NEW_5HP
    local new_trigs = data:clone_triggers(608, 905, RYU_ACTION.NEW_5HP)
...
- Use the Moveset Research window to see what's available to edit. The contents shown under `[Lua Data]` are exactly what you can access from within the character Lua files from the `data` parameter.
