...
local function imgui_options()
	...
	changed, options.no_parry = imgui.checkbox("No Parry ", options.no_parry); wc = wc or changed
	changed, options.no_perfect_parry = imgui.checkbox("No Perfect Parry", options.no_perfect_parry); wc = wc or changed
	
	if wc then 
		json.dump_file("MMDK\\No Parry.json", options)
	end
	imgui.end_rect(2)
end
...
local function apply_moveset_changes(data)
	...
	for name, move_tbl in pairs(moves_by_name) do
		if name:find("DPA_STD_START") and move_tbl.trigger then 
			if options.no_perfect_parry then
				move_tbl.DamageCollisionKey[1]:set_StartFrame(2)
			end
			if options.no_parry then
